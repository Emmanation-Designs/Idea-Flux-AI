      ...
      <div className="w-full flex-1 flex flex-col justify-end gap-3 sm:gap-4 max-w-2xl relative z-30 min-h-0">
        ...
        <div className="flex-1 w-full flex flex-col justify-end gap-3 sm:gap-4 overflow-y-auto scrollbar-hide py-2 min-h-0">
          ...
          <div className="w-full flex flex-col gap-1 pl-1 shrink-0">
            ...
            {/* Card 1: Write or edit */}
            <button
              onClick={() => handleActionClick('write')}
              className="flex items-center gap-4 text-left text-zinc-700 dark:text-zinc-200 hover:text-zinc-900 dark:hover:text-zinc-50 hover:bg-zinc-100/50 dark:hover:bg-zinc-800/40 border border-transparent px-3 py-2.5 sm:py-2 rounded-2xl transition-all duration-200 select-none group w-full"
            >
              <div className="w-6 h-6 flex items-center justify-center shrink-0">
                <Edit3 className="w-5.5 h-5.5 text-[#19C37D] dark:text-[#19C37D] group-hover:scale-105 transition-transform" />
              </div>
              <span className="text-[15px] sm:text-base font-semibold tracking-tight">Write or edit</span>
            </button>

            {/* Card 2: Create an image */}
            <button
              onClick={() => handleActionClick('image')}
              className="flex items-center gap-4 text-left text-zinc-700 dark:text-zinc-200 hover:text-zinc-900 dark:hover:text-zinc-50 hover:bg-zinc-100/50 dark:hover:bg-zinc-800/40 border border-transparent px-3 py-2.5 sm:py-2 rounded-2xl transition-all duration-200 select-none group w-full"
            >
              <div className="w-6 h-6 flex items-center justify-center shrink-0">
                <ImageIcon className="w-5.5 h-5.5 text-[#19C37D] dark:text-[#19C37D] group-hover:scale-105 transition-transform" />
              </div>
              <span className="text-[15px] sm:text-base font-semibold tracking-tight">Create an image</span>
            </button>
            ...
            >
              <div className="w-6 h-6 flex items-center justify-center shrink-0">
                <Globe className="w-5.5 h-5.5 text-[#19C37D] dark:text-[#19C37D] group-hover:scale-105 transition-transform" />
              </div>
              <span className="text-[15px] sm:text-base font-semibold tracking-tight">Look something up</span>
            </button>
          </div>
        </div>
        ...
        <div className="w-full shrink-0 mt-auto">
          ...
          {/* Fully compliant styled pill matching images exactly (Single-row layout) */}
          <div className="bg-zinc-100 dark:bg-[#1E1F22] border border-zinc-200/40 dark:border-zinc-800/60 rounded-full shadow-lg focus-within:ring-2 focus-within:ring-zinc-900/5 dark:focus-within:ring-white/5 dark:focus-within:border-zinc-700 transition-all flex items-center px-2 sm:px-3 py-1.5 min-h-[48px] sm:min-h-[52px] relative overflow-visible">
